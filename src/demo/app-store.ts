...
import {registerAppExtension, registerContributionPoint, updateContext} from "@/core";
...
function activate(ctx: ExtensionContext) {
    ...
    ctx.subscriptions.push(
        registerCommand("app.selectView", selectView)
    );

    ctx.subscriptions.push(
        registerCommand("app.clearView", clearView)
    );

    ctx.subscriptions.push(
        registerCommand("app.updateContext", updateContext)
    );
}
...
registerAppExtension(appManifest, {activate}, {modulePathResolver: moduleResolver});
