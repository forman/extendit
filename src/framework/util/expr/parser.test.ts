...
describe("primary expressions", () => {
  ...
  test("string literals", () => {
    ...
    expect(Parser.parse("'who\\'s there'").toString()).toEqual(
      "string(who's there)"
    );
  });
  ...
  test("other literals", () => {
    ...
    expect(Parser.parse("undefined").toString()).toEqual(
      "undefined(undefined)"
    );
  });
...
describe("prefix expressions", () => {
  ...
  test("index reference", () => {
    expect(Parser.parse("array[13]").toString()).toEqual(
      "idx(array, number(13))"
    );
    expect(Parser.parse("ctx['id']").toString()).toEqual(
      "idx(ctx, string(id))"
    );
  ...
  test("function calls", () => {
    ...
    expect(Parser.parse("angle(x, y)").toString()).toEqual(
      "call(angle, [x, y])"
    );
  });
});
...
describe("unary expressions", () => {
  test("-A", () => {
    expect(Parser.parse("-A").toString()).toEqual("neg(A)");
  });

  test("!A", () => {
    expect(Parser.parse("!A").toString()).toEqual("not(A)");
  });
});
...
describe("conditional expressions", () => {
  test("A ? B : C", () => {
    expect(Parser.parse("A ? B : C").toString()).toEqual("cond(A, B, C)");
  });
  test("A ? B : C ? D : E", () => {
