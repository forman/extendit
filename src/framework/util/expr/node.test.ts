...
test('Conditional', () => {
    let node = new Conditional(new Literal(true), new Literal('hello'), new Literal(8));
    expect(node.eval(ctx)).toEqual("hello");
    expect(node.toString()).toEqual("cond(boolean(true), string(hello), number(8))");

    node = new Conditional(new Literal(false), new Literal('hello'), new Literal(8));
    expect(node.eval(ctx)).toEqual(8);
    expect(node.toString()).toEqual("cond(boolean(false), string(hello), number(8))");
})
