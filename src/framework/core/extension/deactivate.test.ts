...
test('deactivateExtension with failing deactivate()', async () => {
    const {manifest, moduleResolver} = readTestManifest("will-fail-on-deactivation");
    ...
    expect(`${extension.reasons![0]}`).toMatch("Failed by intention: pippo.will-fail-on-deactivation");
    disposable.dispose();
})
