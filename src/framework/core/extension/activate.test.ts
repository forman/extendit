...
import {getExtension, getExtensionContext, setExtensionStatus} from "@/core/store";
...
test('activateExtension that exports Foo API', async () => {
    ...
    expect(ctx.modulePath).toEqual("src/framework/test/extensions/exports-foo-api/main.js");
...
test('activateExtension with dependencies', async () => {
    const {manifest: manifest1, moduleResolver: moduleResolver1} = readTestManifest("exports-baz-api");
    const {manifest: manifest2, moduleResolver: moduleResolver2} = readTestManifest("exports-foo-api");
    const {manifest: manifest3, moduleResolver: moduleResolver3} = readTestManifest("requires-foo-and-baz-apis");
...
test('activateExtension fails on activation', async () => {
    const {manifest, moduleResolver} = readTestManifest("will-fail-on-activation");
    const disposable = registerExtension(manifest, moduleResolver);
    ...
    expect(`${extension.reasons![0]}`).toMatch("Failed by intention: pippo.will-fail-on-activation");
    disposable.dispose();
})

test('activateExtension fails for failing dependency', async () => {
    const {manifest: manifest1, moduleResolver: moduleResolver1} = readTestManifest("will-fail-on-activation");
    const {manifest: manifest2, moduleResolver: moduleResolver2} = readTestManifest("requires-failing");
    const disposable1 = registerExtension(manifest1, moduleResolver1);
    const disposable2 = registerExtension(manifest2, moduleResolver2);
