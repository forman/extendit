import {useContext, useContributions} from "@/core";
...
export function useCommandPalette(): CommandMenuItem[] {
    ...
    return useMemo(() => {
        ...
        return sortMenuItems(
            newMenuItems(commands, menuItems, ctx)
        );
    }, [commands, menuItems, ctx])
}

function useCommandPaletteItems() {
    const menuItems = useContributions<ProcessedMenuItem>(menusPoint.id, COMMAND_PALETTE_MENU_ID);
    return useMemo(() => {
        const commandItemsMap = new Map<string, ProcessedCommandMenuItem>();
        menuItems.forEach(menuItem => {
            if (menuItem.command) {
                commandItemsMap.set(menuItem.command, {...menuItem, command: menuItem.command});
            }
        });
...
function newMenuItems(commands: Command[], paletteItems: Map<string, ProcessedCommandMenuItem>, ctx: Record<string, unknown>) {
    const menuItems: CommandMenuItem[] = [];
    commands.forEach(command => {
        const menuItem = paletteItems.get(command.command!);
        const excluded = menuItem
            && menuItem.when
            && !menuItem.when(ctx);
        if (!excluded) {
